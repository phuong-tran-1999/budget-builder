...
const generateVariableColors = (theme) => {
    ...
    const customPropertiesWithOpacity =
        ...
        ({ opacityVariable, opacityValue }) => {
            if (opacityValue) {
                return `rgba(var(--red-${name}-rgb), ${opacityValue})`;
            }
            if (opacityVariable) {
                return `rgba(var(--red-${name}-rgb), var(${opacityVariable}, 1))`;
            }
            return `rgb(var(--red-${name}-rgb))`;
        };

    return _.fromPairs(
        _.flatten(
            _.map(
                _.keys(flattenColorPalette(normalizeTheme(theme))),
                (name) => [
            ...
            const schemeCustomProps = _.map(
                ...
                (colorScheme) => {
                    ...
                    const background = theme(
                        `redoc.customProps.background.${colorScheme}`
                    );
                    const foreground = theme(
                        `redoc.customProps.foreground.${colorScheme}`
                    );
